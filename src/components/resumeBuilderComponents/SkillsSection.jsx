...
const SkillsSection = ({ resumeData, setResumeData, targetRole, validationErrors = {}, clearValidationErrors }) => {
  const classes = useStyles();
  const [newSkill, setNewSkill] = useState('');
  const [filteredRecommendations, setFilteredRecommendations] = useState([]);
  ...
  const handleAddSkill = () => {
    if (newSkill.trim() !== '' && !resumeData.skills.includes(newSkill.trim())) {
      // Clear validation errors when user adds a skill
  ...
  return (
    <Box className={classes.form}>
      ...
      <TextField
        ...
        helperText={validationErrors['skills'] || "Add your technical and professional skills"}
        InputProps={{
          endAdornment: (
            <InputAdornment position="end">
              <IconButton 
      ...
      {/* Remove the separate error text since it's now in the field */}
