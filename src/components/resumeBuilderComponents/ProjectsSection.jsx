...
const ProjectsSection = ({ resumeData, setResumeData, targetRole, validationErrors = {}, clearValidationErrors }) => {
  const classes = useStyles();
  const [successMessage, setSuccessMessage] = useState('');
  const [loadingProject, setLoadingProject] = useState(false);
  const [projectError, setProjectError] = useState(null);
  ...
  const handleAddRecommendedProject = (projectKey) => {
    ...
    try {
      ...
      if (projectDetails) {
        // Clear validation errors when adding a project
        if (clearValidationErrors) {
          clearValidationErrors();
        }
      ...
      {resumeData.projects.map((project, index) => (
        ...
        >
          ...
          <TextField
            ...
            helperText={validationErrors[`projects.${index}.name`] || ''}
          />
          ...
          <TextField
            ...
            placeholder="e.g., React, Node.js, MongoDB"
            required
            error={!!validationErrors[`projects.${index}.skills_used`]}
            helperText={validationErrors[`projects.${index}.skills_used`] || ''}
          />
          ...
          <TextField
            ...
            helperText={validationErrors[`projects.${index}.description`] || 'Each line will be converted into a bullet point on your resume'}
          />
        </Paper>
      ))}
