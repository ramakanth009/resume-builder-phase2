...
const TwoColumnPDFTemplate = ({ resumeData }) => {
  ...
  const hasWorkExperienceData = () => {
    return resumeData.work_experience && resumeData.work_experience.length > 0 && 
  ...
  const hasSkillsData = () => {
    return resumeData.skills && resumeData.skills.length > 0 && 
           resumeData.skills.some(skill => skill && skill.trim() !== '');
  };
        ...
        <View style={styles.section}>
          ...
          <View style={styles.contactSection}>
            ...
            {resumeData.header.location && (
              <Text style={styles.contactItem}>Location: {resumeData.header.location}</Text>
            )}
            {resumeData.header.website && (
              <Text style={styles.contactItem}>
                Website: <Link src={formatUrl(resumeData.header.website)} style={styles.contactLink}>
                  {resumeData.header.website.replace(/^https?:\/\//, '')}
                </Link>
              </Text>
            )}
            {resumeData.header.linkedin && (
        ...
        {hasWorkExperienceData() && (
          <View style={styles.section}>
            ...
            {resumeData.work_experience
              .filter(exp => (exp.position && exp.position.trim() !== '') || (exp.company && exp.company.trim() !== ''))
              .map((exp, index) => (
                <View key={index} style={styles.experienceItem}>
                  ...
                  {exp.responsibilities && exp.responsibilities.length > 0 && (
                    <View style={styles.bulletList}>
                      {exp.responsibilities.map((resp, respIndex) => (
                        <Bullet key={respIndex}>{resp}</Bullet>
                      ))}
                    </View>
                  )}
                </View>
