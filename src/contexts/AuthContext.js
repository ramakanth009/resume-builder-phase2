...
export const AuthProvider = ({ children }) => {
  ...
  /**
   * Handle OAuth login (Google) callback
   * @param {URLSearchParams} urlParams - URL search parameters from OAuth callback
   * @returns {Object} Response object with status and user data
   */
  const handleOAuthLogin = async (urlParams) => {
    setLoading(true);
    
    try {
      // Parse OAuth callback data using utility function
      ...
      return {
        ...
        loginMethod: oauthData.loginMethod,
  ...
  const value = {
    ...
    register,
    logout,
    handleOAuthLogin // Add OAuth login handler
  };
